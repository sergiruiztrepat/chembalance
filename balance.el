;;; balance.el --- Balances chemical equations - * -lexical-binding: t:- * -
;; -*- coding: utf-8 -*-
;;
...
;; Homepage: https://github.com/sergiruiztrepat/balance.el
;; Package-Requires: ((emacs "24.4"))
;;
;; Balance is free software; you can redistribute it and/or modify it
...
;; balance.  It will give you the balanced equation.
;;
...
;;
;; balance-insert-string (if non-nil, when you call balance with
;; selected region, balance will kill that region and insert the
;; balanced reaction).
;;
...
(defun balance (&optional data)
  ...
  (let* ((data (if data
    ...
    (setq reactives-coeficients (mapcar #'string-to-number
					(car (balance-coeficients reactives))))
    (setq products-coeficients (mapcar #'string-to-number
				       (car (balance-coeficients products))))
    (setq reactives (cadr (balance-coeficients reactives)))
    (setq products (cadr (balance-coeficients products)))

;;    process strings to their parts
    
    (setq reactives-process (mapcar #'balance-process-compound-string
				    reactives))
    (setq products-process (mapcar #'balance-process-compound-string
				  products))
    ;;Is balanced?
    (if	(balance-is-balanced
	 (balance-total-with-coeficients reactives-process
					 reactives-coeficients)
	 (balance-total-with-coeficients  products-process
					  products-coeficients))
	(message "Reaction is balanced.")
      
      ;; Is not balanced:
      (setq reactives-hash (balance-total-by-element
			   reactives-process))
      (setq products-hash (balance-total-by-element
			  products-process))
      (setq process-matrix (balance-process-matrix
			    (balance-matrix reactives-process
					    reactives-hash)
			    (balance-matrix products-process
					    products-hash)))
            
      (when (not (balance-check-reaction reactives-hash products-hash))
	(user-error "Error: reactives and products elements are not the same? "))
      ;; Calculation
      (calc nil nil nil)
      (if calc-prefer-frac
	  (setq results (balance-to-integers
			 (balance-calc-results process-matrix)))
    ...
    (when results
      (setq output
	    (balance-concat-results reactives products results
				    arrow)))
    (if (and balance-insert-string
	     (region-active-p)
...
(defun balance-check-reaction (reactives-hash products-hash)
...
(defun balance-concat-results (reactives products coeficients arrow)
...
(defun balance-matrix (compounds hash)
...
(defun balance-process-matrix (m1 m2)
  "Process matrix M1 and M2 to prepare them to lineal algebra.
M1 comes from (balance-matrix reactives-process reactives-hash).
M2 comes from (balance-matrix products-process products-hash)."
...
(defun balance-is-balanced (reactives products)
...
(defun balance-total-with-coeficients (compounds coeficients)
...
(defun balance-process-compound-string (compound)
  "Process input string COMPOUND and return a list ready to calculate."
  (setq compound (mapcar #'char-to-string compound))
  (setq compound (balance-subst-numbers
		  (balance-insert-1
		   (balance-join-numbers
		    (balance-join-letters
		     (balance-clean-special-chars compound))))))
  ...
  (when (member "(" compound)
    (when (not (member ")" compound))
      (user-error "Error: lacks closing parentheses in formula? "))
    (setq compound (balance-cancel-parens compound)))
  
  (dolist (c compound)
    (when (and (stringp c)
	       (not (balance-number-p c))
	       (not (member c balance-elements)))
      (user-error "Error: %s is not an element" c)))
  compound)

(defun balance-clean-special-chars (elements)
  "Clean list ELEMENTS of special characters like dashes or spaces."
  (let (elements-aux)
    (while elements
      (when (not (member (car elements) '(" " "-" "_" "{" "}")))
	  (push (car elements) elements-aux))
      (setq elements (cdr elements)))
    (reverse elements-aux)))
  
(defun balance-join-letters (elements)
  ...
  (let (first second elements-aux)
    (while elements
      ...
      (if (and
	   (balance-upcase-p first)
	   (balance-downcase-p second))
	  (progn
...
(defun balance-join-numbers (elements)
  ...
  (let (first second elements-aux)
    (while elements
      ...
      (setq second (cadr elements))
      (if (and
	   (balance-number-p first)
	   (balance-number-p second))
	  (progn
	    (setq elements-aux (cons (concat first second) elements-aux))
	    (setq elements (cddr elements)))
	(setq elements-aux (cons first elements-aux))
...
(defun balance-insert-1 (elements)
  ...
  (let (first second elements-aux)
    (while elements
      ...
      (setq second (cadr elements))
      (if (and
	   (not (balance-number-p first))
	   (not (balance-paren-p first))
	   (not (balance-number-p second)))
	  (progn
	    (setq elements-aux (cons first elements-aux))
...
(defun balance-coeficients (compounds-string)
...
(defun balance-cut-list-in (list first last)
...
(defun balance-cut-list-out (list first last)
...
(defun balance-coeficient (compound)
  "Return coeficient after closing parenthesis in COMPOUND."
  (cadr (member ")" compound)))

(defun balance-cancel-parens (compound)
  "Cancel parenthesis in COMPOUND by multiplying with coeficient."
  (let ((inside (balance-cut-list-in compound "(" ")"))
	(outside (balance-cut-list-out compound "(" ")"))
	(coeficient (balance-coeficient compound))
...
(defun balance-string-to-calc (matrix)
  "Make string to ‘calc-eval’ from MATRIX.
MATRIX comes from (balance-process-matrix m1 m2)."
  (let ((m1 (car matrix))
	(m2 (cadr matrix))
...
(defun balance-square-matrix (matrix)
...
(defun balance-calc-results (matrix)
  ...
  (let (square-matrix
    ...
    (while (or (not results)
      ...
      (setq square-matrix (balance-square-matrix matrix))
      (setq string-to-calc (balance-string-to-calc square-matrix))
...
Using lcm of their denominators"
  (let ((lcm (number-to-string (balance-lcm solutions)))
	numbers
	string-to-calc
	results)
